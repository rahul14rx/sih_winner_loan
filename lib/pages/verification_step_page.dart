...
//
// HEAVY UI (wizard-like look) + PER-STEP FLOW
// - Uses the same props as your current StepPage: (loanId, userId, step)
// - Keeps: movement/photo/video capture, AI gate for processId==1,
//          location security, offline queue, upload contract
// - Adds: GPS confidence bar, rich step card, Do's & Don'ts banner placeholder,
//         preview (captured/queued/server), tap-to-enlarge, recapture row
//
// NOTE: Do's & Don'ts banner tries to load: assets/dos/dos_donts_sample.png
// If you don't have that asset, it will gracefully show a placeholder.
...
class _VerificationStepPageState extends State<VerificationStepPage> {
  ...
  Widget _gpsBar() {
    ...
    Color c;
    if (conf >= 75) c = Colors.green;
    else if (conf >= 40) c = Colors.orange;
    else c = Colors.red;
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(_locationStatus, style: GoogleFonts.inter(fontWeight: FontWeight.w800, fontSize: 14)),
          ...
          const SizedBox(width: 10),
          Text("${conf.toStringAsFixed(0)}%", style: GoogleFonts.inter(fontWeight: FontWeight.w900)),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Image.asset(
        ...
        errorBuilder: (_, __, ___) => Center(
          child: Text("Do's & Don'ts Banner", style: GoogleFonts.inter(color: Colors.grey[600], fontWeight: FontWeight.w700)),
        ),
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 10),
          Expanded(
            child: Text(t, style: const TextStyle(color: Color(0xFF856404), fontWeight: FontWeight.w800)),
          ),
        ],
      ),
    );
  ...
  Widget _mediaBox() {
    ...
    if (_mediaFile != null) {
      if (dt == 'video' || dt == 'movement') {
        preview = Container(color: Colors.black, child: const Center(child: Icon(Icons.play_circle, color: Colors.white, size: 60)));
      } else {
      ...
      preview = Center(
        child: Column(
          ...
          children: [
            Icon(dt == 'video' || dt == 'movement' ? Icons.videocam_outlined : Icons.camera_alt_outlined, size: 44, color: Colors.grey[700]),
            const SizedBox(height: 8),
            Text(
              dt == 'video' || dt == 'movement' ? "Tap to record" : "Tap to capture",
              style: GoogleFonts.inter(color: Colors.grey[700], fontWeight: FontWeight.w900),
            ),
            const SizedBox(height: 6),
            Text("Follow on-screen instructions", style: GoogleFonts.inter(color: Colors.grey[600], fontSize: 12)),
          ],
        ),
      );
    }
    ...
    return Column(
      children: [
        GestureDetector(
          onTap: (_uploading || _checkingAi) ? null : () async {
            if (!hasPreview) {
              await _pickMedia();
              return;
            }
                    ...
                    child: Row(
                      children: [
                        Icon(
                          _mediaFile != null
                              ? Icons.check_circle
                              : (showLocalQueued ? Icons.wifi_off : (showServer ? Icons.cloud_done : Icons.error_outline)),
                          size: 16,
                          color: _mediaFile != null
                              ? Colors.green
                              : (showLocalQueued ? Colors.orange : (showServer ? Colors.blue : Colors.orange)),
                        ),
                        const SizedBox(width: 6),
                        Text(
                          _mediaFile != null ? "Captured" : (showLocalQueued ? "Queued" : (showServer ? "Server" : "Not captured")),
          ...
          Container(
            ...
            child: Row(
              children: [
                Expanded(
                  child: Text(
                    (dt == 'video' || dt == 'movement') ? "Preview ready" : "Tap preview to enlarge",
                    style: GoogleFonts.inter(fontWeight: FontWeight.w900),
                  ),
                ),
                OutlinedButton(
                  onPressed: (_uploading || _checkingAi) ? null : _pickMedia,
                  ...
                  child: Text("Recapture", style: GoogleFonts.inter(fontWeight: FontWeight.w900)),
                ),
              ],
            ),
          )
  ...
  Future<void> _submitStep() async {
    ...
    try {
      ...
      final request = http.MultipartRequest('POST', Uri.parse(_url('upload')))
        ...
        ..fields['location_confidence'] = (_sec?.confidence ?? 0).toString();

      if (_needsUtil) {
        request.fields['utilization_amount'] = _amountController.text.trim();
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          widget.step.whatToDo ?? "Verification Step",
          maxLines: 1,
          ...
          FutureBuilder<bool>(
            ...
            builder: (_, snap) {
              ...
              return Padding(
                ...
                child: Row(
                  children: [
                    Icon(online ? Icons.wifi : Icons.wifi_off, color: online ? Colors.greenAccent : Colors.white70),
                    const SizedBox(width: 10),
                    FutureBuilder<int>(
                      future: _queuedCount(),
                      builder: (_, s) {
                        ...
                        return Container(
                          ...
                          child: Text("$c offline", style: GoogleFonts.inter(fontWeight: FontWeight.w800, fontSize: 12)),
                        );
                      },
                    ),
                  ],
                        ...
                        children: [
                          CircleAvatar(
                            ...
                            child: Text(
                              ...
                              style: GoogleFonts.inter(fontWeight: FontWeight.w900, color: _navy),
                            ),
                          ),
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text("Loan: ${widget.loanId}", style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
                                const SizedBox(height: 4),
                                Text("Capture: $dt", style: GoogleFonts.inter(color: Colors.grey[700], fontSize: 12)),
                              ],
                            ),
                          ),
                          Container(
                            ...
                            child: Text(
                              _statusText(),
                              style: GoogleFonts.inter(fontWeight: FontWeight.w800, color: _statusColor()),
                            ),
                          ),
                        ],
                      ),
                    ...
                    Container(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(
                              widget.step.whatToDo ?? "Capture evidence",
                              style: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.w900),
                            ),
                            const SizedBox(height: 8),
                            Text(
                              "Please capture a $dt of the asset as instructed.",
                            ...
                            if (_needsUtil) ...[
                              Text("Utilization amount (₹)", style: GoogleFonts.inter(fontWeight: FontWeight.w900)),
                              const SizedBox(height: 8),
                              TextField(
                            ...
                            Text("Do's & Don'ts", style: GoogleFonts.inter(fontWeight: FontWeight.w900, fontSize: 16)),
                            const SizedBox(height: 10),
                            _dosDontsBanner(),
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton(
                                onPressed: (_uploading || _checkingAi) ? null : _submitStep,
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: (_mediaFile == null) ? Colors.grey.shade300 : _accent,
