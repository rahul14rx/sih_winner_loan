...
class _VerificationStepPageState extends State<VerificationStepPage> with TickerProviderStateMixin {
  ...
  Future<void> _completeWizard() async {
    ...
    try {
      ...
      if (_service.finalizeVerification != null) {
        final ok = await _service.finalizeVerification(widget.loanId);
      ...
      // For each pending utilization (for steps where user entered), call service to save stage utilization.
      // Implementation uses service.saveStageUtilization if available; otherwise we try DatabaseHelper.updateStepUtilization directly.
      try {
        for (final entry in Map<String,double>.from(_pendingUtilizations).entries) {
          ...
          // Prefer service endpoint if exists
          try {
            if (_service.saveStageUtilization != null && step.processId != 0) {
              // Many backends expect stage_no; here we pass processId (step.processId) — adjust if your backend differs.
              await _service.saveStageUtilization(widget.loanId, widget.userId, step.processId, amount);
            } else {
                ...
                child: Container(
                  ...
                  child: Column(children: [
                    ...
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 16),
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text('Do\'s & Don\'ts', style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
                        const SizedBox(height: 8),
                        ...
                        ConstrainedBox(
                          ...
                          child: Container(
                            ...
                            child: Center(child: Text('Banner placeholder (Do\'s & Don\'ts)', style: GoogleFonts.inter(color: Colors.grey[600]))),
                          ),
                        ),
                      ]),
                    ),
  ...
  Widget _buildHeader() {
    final totalUsed = double.tryParse(_loan?.totalUtilized?.toString() ?? "") ?? 0.0;
    final total = double.tryParse(_loan?.amount?.toString() ?? "") ?? 0.0;
  ...
  void _showUtilizationDetails() {
    final used = double.tryParse(_loan?.totalUtilized?.toString() ?? "0") ?? 0.0;
    final total = double.tryParse(_loan?.amount?.toString() ?? "0") ?? 0.0;
